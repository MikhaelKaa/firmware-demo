...
- [ ] Все тесты проходят успешно (`make test_cli` завершается с exit code 0)
- [ ] Сборка без предупреждений при `-Wall -Wextra -Wpedantic -Werror`
- [ ] Покрытие высокоприоритетных тестов: 100%
...
|----------|----------|--------|
| Ввод команд | Ввод `help`, `mem`, `time` и других команд работает корректно | [ ] |
| Навигация | Стрелки влево/вправо перемещают курсор по строке | [ ] |
| Home/End | Клавиши Home/End (или ^A/^E) работают | [ ] |
| История | Up/Down прокручивают историю команд | [ ] |
| Редактирование | Backspace/Delete/^U/^K удаляют символы корректно | [ ] |
| ESC таймаут | Быстрый ввод ESC как обычного символа не ломает парсер | [ ] |
| Автодополнение | Tab (если настроено) работает | [ ] |
| SIGINT | Ctrl+C вызывает обработчик без зависаний | [ ] |
